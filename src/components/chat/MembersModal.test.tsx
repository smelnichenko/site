...
describe('MembersModal', () => {
  ...
  it('kicks member on button click', async () => {
    ...
    render(
      <MembersModal channelId={1} channelName="general" onClose={vi.fn()} onKicked={onKicked} />,
    );
  ...
  it('calls onClose when Close clicked', async () => {
    ...
    const user = userEvent.setup();
    render(
      <MembersModal channelId={1} channelName="general" onClose={onClose} onKicked={vi.fn()} />,
    );
    await waitFor(() => expect(api.fetchChannelMembers).toHaveBeenCalled());
