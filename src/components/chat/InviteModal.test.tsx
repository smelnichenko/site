...
describe('InviteModal', () => {
  it('renders modal with channel name', async () => {
    ...
    render(
      <InviteModal channelId={1} channelName="general" onClose={vi.fn()} onInvited={vi.fn()} />,
    );
  ...
  it('invites user on button click', async () => {
    ...
    render(
      <InviteModal channelId={1} channelName="general" onClose={vi.fn()} onInvited={onInvited} />,
    );
  ...
  it('calls onClose when Close clicked', async () => {
    ...
    const user = userEvent.setup();
    render(
      <InviteModal channelId={1} channelName="general" onClose={onClose} onInvited={vi.fn()} />,
    );
    await waitFor(() => expect(api.fetchChatUsers).toHaveBeenCalled());
