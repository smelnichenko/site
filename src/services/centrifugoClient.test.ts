...
function makeSubscription(opts?: { getToken?: () => Promise<string> }): MockSubscription {
  ...
  const sub: MockSubscription = {
    ...
    handlers,
    on: vi.fn((event: string, handler: Handler) => { handlers[event] = handler; return sub }),
    subscribe: vi.fn(() => { sub.state = 'subscribing' }),
    unsubscribe: vi.fn(() => { sub.state = 'unsubscribed' }),
  }
...
function makeCentrifuge(opts: { getToken: () => Promise<string> }): MockCentrifuge {
  ...
  const client: MockCentrifuge = {
    ...
    handlers,
    on: vi.fn((event: string, handler: Handler) => { handlers[event] = handler; return client }),
    connect: vi.fn(),
    disconnect: vi.fn(),
