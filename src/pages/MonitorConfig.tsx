...
import {
  ...
  createRssFeedMonitor,
  updateRssFeedMonitor,
  deleteRssFeedMonitor,
  testPageMonitor,
  testRssFeedMonitor,
  generateRssCollections,
} from '../services/api';
        ...
        <>
          <span style={{ color: r.matched ? '#155724' : '#856404' }}>
            ...
            {r.extractedValue != null && (
              <>
                {' '}
                — Value: <strong>{r.extractedValue}</strong>
              </>
            )}
            {r.rawMatch && (
              <>
                {' '}
                (raw: <code>{r.rawMatch}</code>)
              </>
            )}
          </span>
          <span style={{ color: '#666', marginLeft: 12 }}>
            HTTP {r.httpStatus} · {r.responseTimeMs}ms
          </span>
        </>
      )}
    </div>
  ...
  return (
    <div
      style={{
        ...
        fontSize: '0.85rem',
        marginBottom: 8,
        background: r.errorMessage ? '#f8d7da' : '#d4edda',
      }}
    >
      {r.errorMessage ? (
        <span style={{ color: '#721c24' }}>Error: {r.errorMessage}</span>
...
function PageMonitorForm({
  ...
  const handleTest = async () => {
    ...
    try {
      setFormTestResult(await withLoading(() => testPageMonitor(form)));
    } catch {
      /* ignore */
    }
  ...
  return (
    <form className="config-form" onSubmit={handleSubmit} ref={formRef}>
      ...
      <div className="form-row">
        ...
        <div className="form-group">
          <label htmlFor="page-cron">
            Cron <span className="required">*</span>
          </label>
          <input
            id="page-cron"
            value={form.cron}
            ...
            placeholder="0 0 * * * *"
            pattern="(\S+\s+){5}\S+"
            title="Spring cron: 6 fields (sec min hour day month weekday)"
          />
        </div>
      </div>
      <div className="form-group">
      ...
      <div className="form-group">
        <label htmlFor="page-pattern">
          Pattern (regex) <span className="required">*</span>
        </label>
        <input
          id="page-pattern"
          value={form.pattern}
      ...
      <div className="form-actions">
        ...
        <div>
          ...
          />
          <button type="button" className="status-badge action" onClick={onCancel}>
            Cancel
          </button>
          <LoadingButton
            type="submit"
...
function mapGeneratedCollections(
...
function RssFeedForm({
  ...
  const addCollection = () =>
    setForm({ ...form, collections: [...form.collections, { name: '', metrics: [] }] });
  const removeCollection = (i: number) =>
    setForm({ ...form, collections: form.collections.filter((_, idx) => idx !== i) });
    ...
    <form className="config-form" onSubmit={handleSubmit} ref={formRef}>
      ...
      <div>
        <div className="form-group" style={{ width: '10%' }}>
          <label htmlFor="rss-max-articles">
            Max Articles <span className="required">*</span>
          </label>
          <input
            id="rss-max-articles"
            type="number"
            ...
            onChange={(e) =>
              setForm({ ...form, maxArticles: Number.parseInt(e.target.value, 10) || 30 })
            }
            required
          />
        </div>
        <label className="toggle-label">
          <input
            type="checkbox"
            checked={form.fetchContent}
            onChange={(e) => setForm({ ...form, fetchContent: e.target.checked })}
          />{' '}
          Fetch Content
        </label>
      </div>
          ...
          <div style={{ marginTop: 8, padding: '12px', background: '#f8f9fa', borderRadius: 4 }}>
            ...
            <LoadingButton
              ...
              onClick={async () => {
                ...
                setAiError('');
                try {
                  const collections = await withLoading(() =>
                    generateRssCollections({ url: form.url, prompt: aiPrompt }),
                  );
                  setForm({
                    ...form,
            ...
            />
            {!form.url.trim() && (
              <span style={{ color: '#856404', fontSize: '0.85rem', marginLeft: 8 }}>
                Enter a feed URL first
              </span>
            )}
          </div>
        )}
      ...
      <div className="collections-section">
        <div className="section-header">
          ...
          <button type="button" className="status-badge add" onClick={addCollection}>
            + Collection
          </button>
        </div>
        {form.collections.map((col, ci) => (
          <div key={`col-${col.name || ci}`} className="collection-block">
            <div className="form-row">
              <div className="form-group">
                <label htmlFor={`col-name-${ci}`}>
                  Collection Name <span className="required">*</span>
                </label>
                <input
                  id={`col-name-${ci}`}
                  value={col.name}
                  onChange={(e) => updateCollection(ci, { ...col, name: e.target.value })}
                  required
                />
              </div>
              <button
            ...
            {col.metrics.map((m, mi) => (
              <div key={`metric-${ci}-${m.name || mi}`} className="metric-row">
                <input
                  ...
                  value={m.name}
                  onChange={(e) => updateMetric(ci, mi, { ...m, name: e.target.value })}
                  required
                />
                <input
                  placeholder="Keywords (comma-separated)"
                  value={m.keywords}
                  onChange={(e) => updateMetric(ci, mi, { ...m, keywords: e.target.value })}
                  required
                />
                <button
                  type="button"
                  className="status-badge danger"
                  onClick={() => removeMetric(ci, mi)}
                >
                  x
                </button>
              </div>
            ))}
            <button type="button" className="status-badge add" onClick={() => addMetric(ci)}>
              + Metric
            </button>
          </div>
        ))}
...
function buildTestRssRequest(fm: RssFeedMonitorConfig): RssFeedMonitorRequest {
  return {
    ...
    url: fm.url,
    cron: fm.cron,
    fetchContent: fm.fetchContent,
    maxArticles: fm.maxArticles,
    enabled: fm.enabled,
    collections: fm.collections.map((c) => ({
      name: c.name,
      metrics: c.metrics.map((m) => ({ name: m.name, keywords: m.keywords })),
    })),
  };
}
...
function MonitorConfig() {
  ...
  const handleTestPage = async (pm: PageMonitorConfig) => {
    ...
    try {
      await withLoading(() => testPageMonitor(buildTestPageRequest(pm)));
    } catch {
      /* ignore */
    }
    setTesting(null);
      ...
      <div className="card">
        <div className="card-header">
          ...
          {!showNewPage && !editingPage && (
            <button className="status-badge add" onClick={() => setShowNewPage(true)}>
              + Add Page Monitor
            </button>
          )}
        </div>
        ...
        {pageMonitors.length === 0 && !showNewPage ? (
          <p className="empty-state">
            No page monitors configured. Click &quot;+ Add Page Monitor&quot; to create one.
          </p>
        ) : (
                  ...
                  <div className="config-item-row">
                    <div className="config-item-info">
                      ...
                      <span className="config-detail">
                        Pattern: <code>{pm.pattern}</code>
                      </span>
                      <span className="config-detail">
                        Cron: <code>{pm.cron}</code>
                      </span>
                    </div>
                    <div className="config-item-actions">
                      <LoadingButton
                        className="status-badge action"
                        onClick={() => handleTestPage(pm)}
                        loading={testing === 'page:' + pm.name}
                        label="Test"
                        loadingLabel="Testing..."
                      />
                      <button className="status-badge action" onClick={() => setEditingPage(pm)}>
                        Edit
                      </button>
                      <button
                        className="status-badge danger"
                        onClick={() => handleDeletePage(pm.id)}
                      >
                        Delete
                      </button>
                    </div>
      ...
      <div className="card">
        <div className="card-header">
          ...
          {!showNewRss && !editingRss && (
            <button className="status-badge add" onClick={() => setShowNewRss(true)}>
              + Add RSS Feed
            </button>
          )}
        </div>
        ...
        {rssMonitors.length === 0 && !showNewRss ? (
          <p className="empty-state">
            No RSS feed monitors configured. Click &quot;+ Add RSS Feed&quot; to create one.
          </p>
        ) : (
          <div className="config-list">
            {rssMonitors.map((fm) => (
                ...
                {editingRss?.id === fm.id ? (
                  ...
                  <div className="config-item-row">
                    <div className="config-item-info">
                      ...
                      <span className="config-detail">
                        Cron: <code>{fm.cron}</code> | Max articles: {fm.maxArticles}
                      </span>
                    ...
                    <div className="config-item-actions">
                      <LoadingButton
                        className="status-badge action"
                        onClick={() => handleTestRss(fm)}
                        loading={testing === 'rss:' + fm.name}
                        label="Test"
                        loadingLabel="Testing..."
                      />
                      <button className="status-badge action" onClick={() => setEditingRss(fm)}>
                        Edit
                      </button>
                      <button
                        className="status-badge danger"
                        onClick={() => handleDeleteRss(fm.id)}
                      >
                        Delete
                      </button>
                    </div>
