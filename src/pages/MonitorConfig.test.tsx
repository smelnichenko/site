...
describe('MonitorConfig', () => {
  ...
  it('creates a new page monitor via form', async () => {
    vi.mocked(api.fetchPageMonitorConfigs).mockResolvedValue([]);
    vi.mocked(api.fetchRssFeedMonitorConfigs).mockResolvedValue([]);
    vi.mocked(api.createPageMonitor).mockResolvedValue({
      id: 2,
      name: 'My Page',
  ...
  it('saves edit for page monitor', async () => {
    vi.mocked(api.fetchPageMonitorConfigs).mockResolvedValue(mockPageMonitors);
    vi.mocked(api.fetchRssFeedMonitorConfigs).mockResolvedValue([]);
    vi.mocked(api.updatePageMonitor).mockResolvedValue({
      id: 1,
      name: 'Bing',
      url: 'http://google.com',
      pattern: String.raw`\d+`,
    ...
    expect(api.updatePageMonitor).toHaveBeenCalledWith(
      1,
  ...
  it('saves edit for RSS monitor', async () => {
    vi.mocked(api.fetchPageMonitorConfigs).mockResolvedValue([]);
    vi.mocked(api.fetchRssFeedMonitorConfigs).mockResolvedValue(mockRssMonitors);
    vi.mocked(api.updateRssFeedMonitor).mockResolvedValue({
      id: 1,
      name: 'Updated Feed',
      url: 'http://x.com/rss',
      cron: '0 0 * * * *',
    ...
    expect(api.updateRssFeedMonitor).toHaveBeenCalledWith(
      1,
  ...
  it('saves RSS form with collections converting keywords via toCollectionRequests', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('Keywords (comma-separated)'), {
      target: { value: 'ai, ml, deep learning' },
    });
    await user.click(screen.getByText('Save'));

    expect(api.createRssFeedMonitor).toHaveBeenCalledWith(
