...
describe('Chat', () => {
  ...
  it('navigates when selecting a channel', async () => {
    ...
    await userEvent
      ...
      .click(screen.getByText('Select 1'));
  ...
  it('closes modal and reloads channels on channel created', async () => {
    ...
    vi.mocked(api.fetchChatChannels).mockResolvedValue([
      ...mockChannels,
      {
        id: 3,
        name: 'new-channel',
  ...
  it('polls for channels on interval', async () => {
    ...
    await waitFor(() => {
      expect(vi.mocked(api.fetchChatChannels).mock.calls.length).toBeGreaterThan(
        callCountAfterLoad,
      );
    });
  });
